...
func ErrMissingRegistrant(modelName string) error {
	return errors.New(
		...
		[]string{"Meshplay models are always registered in context of a registrant."},
...
func ErrRegisterEntity(err error, name, entity string) error {
	return errors.New(
		...
		errors.Alert,
		[]string{fmt.Sprintf("Failed to register an entity of name: %s and type: %s into Meshplay's registry", name, entity)},
		[]string{err.Error()},
...
func ErrGetEntity(err error) error {
	return errors.New(
		ErrGetEntityCode,
		errors.Alert,
		[]string{"Could not parse the given data into any Meshplay entity"},
		[]string{err.Error()},
		...
		[]string{"Ensure that the definition given is in accordance with it's schema", "Ensure that the schemaVersion used is valid and is supported by Meshplay"},
	)
}
...
func ErrDirPkgUnitParseFail(dirpath string, err error) error {
	return errors.New(
		...
		errors.Alert,
		[]string{fmt.Sprintf("Directory at path: %s cannot be registered into Meshplay", dirpath)},
		...
		[]string{"Make sure that there is a valid model definition present in the directory. Meshplay's registration pipeline currently does not support nested models, therefore the behaviour might be unexpected if it contains nested models.", "If there is an internal error, please try again after some time"},
	)
}
